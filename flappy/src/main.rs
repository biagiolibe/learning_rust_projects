...
impl State {
    fn new() -> Self {
        State {
            player: Player::new(SCREEN_WIDTH / 16, SCREEN_HEIGHT / 2), //vertically centered to the screen (80x50)
            frame_time: 0.0,
            mode: GameMode::Menu,
    ...
    fn dead(&mut self, ctx: &mut BTerm) {
        ...
        if let Some(key) = ctx.key {
            match key {
                ...
                VirtualKeyCode::Q => ctx.quitting = true, //close window
                _ => {}
            }
        }
    }
    ...
    fn restart(&mut self) {
        self.player = Player::new(SCREEN_WIDTH / 16, SCREEN_HEIGHT / 2);
...
// Implementing trait GameState (it is needed for using main_loop function)
...
impl Obstacle {
    fn hit_obstacle(&self, player: &Player) -> bool {
        ...
        player.x == self.x && (player_above_gap || player_below_gap)
    }
}
